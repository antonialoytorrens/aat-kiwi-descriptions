...
# Install Extra Packages (from Makefile.extra downloads)
# If we have .deb files in /tmp/extra_packages (we will place them there via Makefile)
if [ -d /tmp/extra_packages ]; then
    echo "Installing extra packages..."
    apt-get install -y /tmp/extra_packages/*.deb || true
fi
